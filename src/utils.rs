...
static CONFIG_BASEPATH: Lazy<PathBuf> = Lazy::new(|| {
    let mut res = glib::user_config_dir();
    res.push("euphonica");
    res
});
...
/// NOT where app config is stored (those are in dconf or whatever backend gio::Settings use).
/// This is currently used to store the autogenerated config file for the managed MPD instance only.
pub fn get_config_basepath() -> PathBuf {
    CONFIG_BASEPATH.clone()
}

/// NOT where app config is stored (those are in dconf or whatever backend gio::Settings use).
/// This is currently used to store the autogenerated config file for the managed MPD instance only.
pub fn get_standalone_config_path() -> PathBuf {
    let mut res = CONFIG_BASEPATH.clone();
    res.push("mpd.conf");
    res
}

/// Config dir is probably more likely to be backed up by people than the cache dir.
pub fn get_standalone_playlists_path() -> PathBuf {
    let mut res = CONFIG_BASEPATH.clone();
    res.push("playlists");
    res
}
